use melaec3::cpal_aec::{build_chirp_slice, gcc_phat_delay};

const SAMPLE_RATE: usize = 16_000;
const TONE_SECONDS: f32 = 0.1;
const SLICE_SECONDS: f32 = 4.0;
const OFFSET_COUNT: usize = 32;
const TONE_FRAMES: usize = {
    let frames = (TONE_SECONDS * SAMPLE_RATE as f32 + 0.5) as usize;
    if frames < 1 { 1 } else { frames }
};
...
fn gcc_phat_delay_tracks_offsets_across_range() {
    ...
    let base_start = slice_samples / 2 - TONE_FRAMES / 2;
    let max_probe_start = (slice_samples - TONE_FRAMES) as isize;

    let offsets = offset_sweep_frames();
    let base_silence_after = silence_after(base_start, slice_samples, TONE_FRAMES);
    let input = build_chirp_slice(0, base_start, TONE_FRAMES, base_silence_after, SAMPLE_RATE as u32);
    let base_start_idx = base_start;
    ...
    for requested_offset_samples in offsets {
        ...
        let probe_silence_after = silence_after(probe_start, slice_samples, TONE_FRAMES);
        let probe = build_chirp_slice(0, probe_start, TONE_FRAMES, probe_silence_after, SAMPLE_RATE as u32);
        let probe_start_idx = probe_start;
...
fn gcc_phat_delay_with_noise_still_tracks_offsets() {
    ...
    for requested_offset_samples in offsets {
        ...
        let probe_start = desired_probe_start.clamp(0, max_probe_start) as usize;
        let probe_silence_after = silence_after(probe_start, slice_samples, TONE_FRAMES);
        let mut probe = build_chirp_slice(0, probe_start, TONE_FRAMES, probe_silence_after, SAMPLE_RATE as u32);
        let probe_start_idx = probe_start;
