...
unsafe extern "C" fn compute_func(
    ...
    interp[1 as std::ffi::c_int as usize] = 1.0f64
        - interp[3 as std::ffi::c_int as usize] - interp[2 as std::ffi::c_int as usize]
...
unsafe extern "C" fn sinc(
    ...
    return (cutoff as std::ffi::c_double * sin(M_PI * xx as std::ffi::c_double)
        ...
        * compute_func(
            fabs(2.0f64 * x as std::ffi::c_double / N as std::ffi::c_double)
                as std::ffi::c_float,
            window_func,
        )) as spx_word16_t;
}
unsafe extern "C" fn cubic_coef(mut frac: spx_word16_t, mut interp: *mut spx_word16_t) {
    *interp.offset(0 as std::ffi::c_int as isize) = -0.16667f32 * frac
        + 0.16667f32 * frac * frac * frac;
    *interp.offset(1 as std::ffi::c_int as isize) = frac + 0.5f32 * frac * frac
        - 0.5f32 * frac * frac * frac;
    *interp.offset(3 as std::ffi::c_int as isize) = -0.33333f32 * frac
        + 0.5f32 * frac * frac - 0.16667f32 * frac * frac * frac;
...
unsafe extern "C" fn resampler_basic_direct_single(
    ...
    let mut last_sample: std::ffi::c_int = *((*st).last_sample)
        .offset(channel_index as isize);
    let mut samp_frac_num: spx_uint32_t = *((*st).samp_frac_num)
        .offset(channel_index as isize);
        ...
        || out_sample as spx_int32_t >= *out_len as spx_int32_t)
    {
        ...
        let mut iptr: *const spx_word16_t = &*in_0.offset(last_sample as isize)
            as *const spx_word16_t;
        ...
        || out_sample as spx_int32_t >= *out_len as spx_int32_t)
    {
        ...
        while j < N {
            accum[0 as std::ffi::c_int as usize]
                += (*sinct.offset(j as isize) * *iptr.offset(j as isize))
                    as std::ffi::c_double;
            accum[1 as std::ffi::c_int as usize]
                += (*sinct.offset((j + 1 as std::ffi::c_int) as isize)
                    * *iptr.offset((j + 1 as std::ffi::c_int) as isize))
                    as std::ffi::c_double;
            accum[2 as std::ffi::c_int as usize]
                += (*sinct.offset((j + 2 as std::ffi::c_int) as isize)
                    * *iptr.offset((j + 2 as std::ffi::c_int) as isize))
                    as std::ffi::c_double;
            accum[3 as std::ffi::c_int as usize]
                += (*sinct.offset((j + 3 as std::ffi::c_int) as isize)
                    * *iptr.offset((j + 3 as std::ffi::c_int) as isize))
                    as std::ffi::c_double;
            j += 4 as std::ffi::c_int;
        }
        sum = accum[0 as std::ffi::c_int as usize] + accum[1 as std::ffi::c_int as usize]
            + accum[2 as std::ffi::c_int as usize]
            + accum[3 as std::ffi::c_int as usize];
...
unsafe extern "C" fn resampler_basic_interpolate_single(
        ...
        || out_sample as spx_int32_t >= *out_len as spx_int32_t)
    {
        ...
        sum = interp[0 as std::ffi::c_int as usize]
            * accum[0 as std::ffi::c_int as usize]
            + interp[1 as std::ffi::c_int as usize]
                * accum[1 as std::ffi::c_int as usize]
            + interp[2 as std::ffi::c_int as usize]
...
unsafe extern "C" fn resampler_basic_interpolate_double(
        ...
        || out_sample as spx_int32_t >= *out_len as spx_int32_t)
    {
        ...
        j = 0 as std::ffi::c_int;
        while j < N {
            let curr_in: std::ffi::c_double = *iptr.offset(j as isize)
                as std::ffi::c_double;
            accum[0 as std::ffi::c_int as usize]
...
unsafe extern "C" fn resampler_basic_zero(
        ...
        || out_sample as spx_int32_t >= *out_len as spx_int32_t)
    {
        ...
        out_sample = out_sample + 1;
        *out.offset((out_stride * fresh4) as isize) = 0 as std::ffi::c_int
            as spx_word16_t;
        last_sample += int_advance;
        samp_frac_num = samp_frac_num.wrapping_add(frac_advance as spx_uint32_t);
        if samp_frac_num >= den_rate {
...
unsafe extern "C" fn multiply_frac(
    ...
    if remain > (UINT32_MAX as spx_uint32_t).wrapping_div(num)
        ...
        || major.wrapping_mul(num)
            > (UINT32_MAX as spx_uint32_t)
                .wrapping_sub(remain.wrapping_mul(num).wrapping_div(den))
    {
...
unsafe extern "C" fn update_filter(mut st: *mut SpeexResamplerState) -> std::ffi::c_int {
    ...
    (*st).frac_advance = ((*st).num_rate).wrapping_rem((*st).den_rate)
        as std::ffi::c_int;
    ...
    if (*st).num_rate > (*st).den_rate {
        (*st).cutoff = quality_map[(*st).quality as usize].downsample_bandwidth
            * (*st).den_rate as std::ffi::c_float / (*st).num_rate as std::ffi::c_float;
        if multiply_frac(
            &mut (*st).filt_len,
            (*st).filt_len,
            (*st).num_rate,
        ...
        15652330335145281839 => {
            use_direct = (((*st).filt_len).wrapping_mul((*st).den_rate)
                ...
                && (INT_MAX as std::ffi::c_ulong)
                    ...
                    >= (*st).filt_len as std::ffi::c_ulong) as std::ffi::c_int;
            if use_direct != 0 {
                ...
                _ => {
                    if (*st).sinc_table_length < min_sinc_table_length {
                        let mut sinc_table: *mut spx_word16_t = speex_realloc(
                            ...
                            (min_sinc_table_length as std::ffi::c_ulong)
                                .wrapping_mul(
                                    ::core::mem::size_of::<spx_word16_t>() as std::ffi::c_ulong,
                                ) as std::ffi::c_int,
                        ) as *mut spx_word16_t;
                        ...
                        _ => {
                            if use_direct != 0 {
                                        ...
                                        *((*st).sinc_table)
                                            .offset(
                                                i
                                                    ...
                                                    .wrapping_add(j as spx_uint32_t) as isize,
                                            ) = sinc(
                                            (*st).cutoff,
                                            (j - (*st).filt_len as spx_int32_t / 2 as spx_int32_t
                                                + 1 as spx_int32_t) as std::ffi::c_float
                                ...
                                while i_0
                                    < ((*st).oversample)
                                        .wrapping_mul((*st).filt_len)
                                        .wrapping_add(4 as spx_uint32_t) as spx_int32_t
                                {
                                    *((*st).sinc_table)
                                        .offset((i_0 + 4 as spx_int32_t) as isize) = sinc(
                                        (*st).cutoff,
                            ...
                            match current_block {
                                ...
                                _ => {
                                    if (*st).started == 0 {
                                        ...
                                        {
                                            *((*st).mem).offset(i_1 as isize) = 0 as std::ffi::c_int
                                                as spx_word16_t;
                                        ...
                                        loop {
                                            ...
                                            let mut start: spx_uint32_t = i_2
                                                .wrapping_mul((*st).mem_alloc_size);
                                            let mut magic_samples: spx_uint32_t = *((*st).magic_samples)
                                                .offset(i_2 as isize);
                                            olen = old_length
                                                .wrapping_add(
                                            ...
                                            loop {
                                                ...
                                                *((*st).mem)
                                                    .offset(
                                                        start.wrapping_add(j_0).wrapping_add(magic_samples) as isize,
                                                    ) = *((*st).mem)
                                                    .offset(
                                                        i_2.wrapping_mul(old_alloc_size).wrapping_add(j_0) as isize,
                                                    );
                                            }
                                            j_0 = 0 as spx_uint32_t;
                                            while j_0 < magic_samples {
                                                *((*st).mem).offset(start.wrapping_add(j_0) as isize) = 0
                                                    as std::ffi::c_int as spx_word16_t;
                                                j_0 = j_0.wrapping_add(1);
                                            }
                                            *((*st).magic_samples).offset(i_2 as isize) = 0
                                                as spx_uint32_t;
                                            if (*st).filt_len > olen {
                                                j_0 = 0 as spx_uint32_t;
                                                while j_0 < olen.wrapping_sub(1 as spx_uint32_t) {
                                                    *((*st).mem)
                                                        ...
                                                        .offset(
                                                            start
                                                                .wrapping_add(
                                                                    olen.wrapping_sub(2 as spx_uint32_t).wrapping_sub(j_0),
                                                                ) as isize,
                                                        );
                                                    j_0 = j_0.wrapping_add(1);
                                                }
                                                while j_0 < ((*st).filt_len).wrapping_sub(1 as spx_uint32_t)
                                                {
                                                    *((*st).mem)
                                                        .offset(
                                                            start
                                                ...
                                                let ref mut fresh7 = *((*st).last_sample)
                                                    .offset(i_2 as isize);
                                                *fresh7 = (*fresh7 as spx_uint32_t)
                                                    .wrapping_add(
                                                        ((*st).filt_len)
                                                            .wrapping_sub(olen)
                                                            .wrapping_div(2 as spx_uint32_t),
                                                    ) as spx_int32_t as spx_int32_t;
                                            } else {
                                                magic_samples = olen
                                                ...
                                                while j_0
                                                    < ((*st).filt_len)
                                                        ...
                                                        .wrapping_add(magic_samples)
                                                {
                                                    *((*st).mem).offset(start.wrapping_add(j_0) as isize) = *((*st)
                                                        .mem)
                                                        .offset(
                                                            start.wrapping_add(j_0).wrapping_add(magic_samples) as isize,
                                                        );
                                                ...
                                                *((*st).magic_samples).offset(i_2 as isize) = magic_samples;
                                            }
                                        }
                                    } else if (*st).filt_len < old_length {
                                        ...
                                        while i_3 < (*st).nb_channels {
                                            ...
                                            let mut old_magic: spx_uint32_t = *((*st).magic_samples)
                                                .offset(i_3 as isize);
                                            *((*st).magic_samples).offset(i_3 as isize) = old_length
                                            ...
                                            while j_1
                                                < ((*st).filt_len)
                                                    .wrapping_sub(1 as spx_uint32_t)
                                                    .wrapping_add(*((*st).magic_samples).offset(i_3 as isize))
                                                    .wrapping_add(old_magic)
                                            {
                                                *((*st).mem)
                                                    .offset(
                                                        i_3.wrapping_mul((*st).mem_alloc_size).wrapping_add(j_1)
                                                            as isize,
                                                    ) = *((*st).mem)
                                                    .offset(
                                                        i_3
                                            ...
                                            let ref mut fresh8 = *((*st).magic_samples)
                                                .offset(i_3 as isize);
...
pub unsafe extern "C" fn speex_resampler_init_frac(
    ...
    if nb_channels == 0 as spx_uint32_t || ratio_num == 0 as spx_uint32_t
        || ratio_den == 0 as spx_uint32_t || quality > 10 as std::ffi::c_int
    ...
    if !((*st).last_sample).is_null() {
        (*st).magic_samples = speex_alloc(
            (nb_channels as std::ffi::c_ulong)
                .wrapping_mul(
                    ::core::mem::size_of::<spx_uint32_t>() as std::ffi::c_ulong,
                ) as std::ffi::c_int,
        ) as *mut spx_uint32_t;
...
unsafe extern "C" fn speex_resampler_process_native(
    ...
    *fresh9 = (*fresh9 as spx_uint32_t).wrapping_sub(*in_len) as spx_int32_t
        as spx_int32_t;
    ilen = *in_len;
    j = 0 as std::ffi::c_int;
    while j < N - 1 as std::ffi::c_int {
        *mem.offset(j as isize) = *mem
            .offset((j as spx_uint32_t).wrapping_add(ilen) as isize);
        j += 1;
    }
    return RESAMPLER_ERR_SUCCESS as std::ffi::c_int;
}
unsafe extern "C" fn speex_resampler_magic(
    ...
    let mut tmp_in_len: spx_uint32_t = *((*st).magic_samples)
        .offset(channel_index as isize);
    let mut mem: *mut spx_word16_t = ((*st).mem)
        .offset(channel_index.wrapping_mul((*st).mem_alloc_size) as isize);
    let N: std::ffi::c_int = (*st).filt_len as std::ffi::c_int;
    ...
    *fresh10 = (*fresh10).wrapping_sub(tmp_in_len);
    if *((*st).magic_samples).offset(channel_index as isize) != 0 {
        ...
        while i < *((*st).magic_samples).offset(channel_index as isize) {
            *mem
                .offset(
                    ((N - 1 as std::ffi::c_int) as spx_uint32_t).wrapping_add(i) as isize,
                ) = *mem
                .offset(
    ...
    *out = (*out)
        .offset(out_len.wrapping_mul((*st).out_stride as spx_uint32_t) as isize);
...
pub unsafe extern "C" fn speex_resampler_process_float(
    ...
    let mut x: *mut spx_word16_t = ((*st).mem)
        .offset(channel_index.wrapping_mul((*st).mem_alloc_size) as isize);
    let filt_offs: std::ffi::c_int = ((*st).filt_len).wrapping_sub(1 as spx_uint32_t)
        as std::ffi::c_int;
    let xlen: spx_uint32_t = ((*st).mem_alloc_size)
        .wrapping_sub(filt_offs as spx_uint32_t);
    let istride: std::ffi::c_int = (*st).in_stride;
    if *((*st).magic_samples).offset(channel_index as isize) != 0 {
        olen = olen
            .wrapping_sub(
                speex_resampler_magic(st, channel_index, &mut out, olen) as spx_uint32_t,
            );
    }
    if *((*st).magic_samples).offset(channel_index as isize) == 0 {
        while ilen != 0 && olen != 0 {
            ...
            if !in_0.is_null() {
                ...
                while (j as spx_uint32_t) < ichunk {
                    *x.offset((j + filt_offs) as isize) = *in_0
                        .offset((j * istride) as isize) as spx_word16_t;
                    j += 1;
                }
            } else {
                j = 0 as std::ffi::c_int;
                while (j as spx_uint32_t) < ichunk {
                    *x.offset((j + filt_offs) as isize) = 0 as std::ffi::c_int
                        as spx_word16_t;
                    j += 1;
                }
            }
            ...
            out = out
                .offset(ochunk.wrapping_mul((*st).out_stride as spx_uint32_t) as isize);
            if !in_0.is_null() {
                in_0 = in_0
                    .offset(ichunk.wrapping_mul(istride as spx_uint32_t) as isize);
            }
        }
    }
...
pub unsafe extern "C" fn speex_resampler_process_int(
    ...
    let mut x: *mut spx_word16_t = ((*st).mem)
        .offset(channel_index.wrapping_mul((*st).mem_alloc_size) as isize);
    let xlen: spx_uint32_t = ((*st).mem_alloc_size)
        .wrapping_sub(((*st).filt_len).wrapping_sub(1 as spx_uint32_t));
    ...
    while ilen != 0 && olen != 0 {
        ...
        if *((*st).magic_samples).offset(channel_index as isize) != 0 {
            omagic = speex_resampler_magic(st, channel_index, &mut y, ochunk)
                as spx_uint32_t;
        ...
        while (j as spx_uint32_t) < ochunk.wrapping_add(omagic) {
            *out.offset((j * ostride_save) as isize) = (if ystack[j as usize]
                < -32767.5f32
            {
        ...
        out = out
            .offset(
                ochunk.wrapping_add(omagic).wrapping_mul(ostride_save as spx_uint32_t)
                    as isize,
            );
        if !in_0.is_null() {
            in_0 = in_0
                .offset(ichunk.wrapping_mul(istride_save as spx_uint32_t) as isize);
        }
    }
...
#[inline]
unsafe extern "C" fn compute_gcd(
    mut a: spx_uint32_t,
    mut b: spx_uint32_t,
) -> spx_uint32_t {
    ...
    if old_den > 0 as spx_uint32_t {
        ...
        while i < (*st).nb_channels {
            ...
            if *((*st).samp_frac_num).offset(i as isize) >= (*st).den_rate {
                *((*st).samp_frac_num).offset(i as isize) = ((*st).den_rate)
                    .wrapping_sub(1 as spx_uint32_t);
            }
            i = i.wrapping_add(1);
        }
...
pub unsafe extern "C" fn speex_resampler_skip_zeros(
    ...
    i = 0 as spx_uint32_t;
    while i < (*st).nb_channels {
        *((*st).last_sample).offset(i as isize) = ((*st).filt_len)
            .wrapping_div(2 as spx_uint32_t) as spx_int32_t;
        i = i.wrapping_add(1);
    }
    return RESAMPLER_ERR_SUCCESS as std::ffi::c_int;
}
...
pub unsafe extern "C" fn speex_resampler_reset_mem(
    ...
    while i
        < ((*st).nb_channels)
            .wrapping_mul(((*st).filt_len).wrapping_sub(1 as spx_uint32_t))
    {
...
pub unsafe extern "C" fn speex_resampler_strerror(
    ...
    match err {
        ...
        4 => {
            return b"Input and output buffers overlap.\0" as *const u8
                as *const std::ffi::c_char;
        }
        _ => {
            return b"Unknown error. Bad error code or strange version mismatch.\0"
